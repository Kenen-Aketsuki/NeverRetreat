...
public class TRATurn : ITurnControl
{
    ...
    public void ActionStageStart()
    {
        ...
        HttpConnect.instance.ResetAIData();
    ...
    IEnumerator AIAgentWork()
    {
        ...
        while (CommandControl.CC.ToActionPiece.Count > 0)
        {
            while (CommandControl.CC.ToActionPiece.Count > 0)
            {
                ...
                if (FaildCommandCount.ContainsKey(currentPiece.name) && FaildCommandCount[currentPiece.name] >= maxCommandTime) continue;
                //前向传播
                isHttpWait = true;
...
public class PVETurn : ITurnControl
{
    #region//第零阶段
    public void ZeroTurnStageStart()
    {
